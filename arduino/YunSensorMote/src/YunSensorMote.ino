...
void buttonCommand(BridgeClient client) {
    getButton();
    sensorJson["sensor"] = "button";
    sensorJson["value"] = button;
    // Send feedback to client
    sensorJson.printTo(client);

    // Update datastore key with the current pin value
    String key = "D0";
    Bridge.put(key, String(button));
}
...
void loop() {
  ...
  BridgeClient BridgeClient = server.accept();
